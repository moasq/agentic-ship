...
export function sanitizeString(value) {
  let result = value;
  for (const pattern of [...CREDENTIALS, ...PERSONAL_DATA]) {
...
export function sanitize(value) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => [
      key,
      PRIVATE_KEYS.test(key) ? "[REDACTED]" : sanitize(item),
    ]),
  );
}
...
function rejectPrivateInput(value) {
  ...
  if (sanitizeString(source) !== source) {
    throw new Error("arguments contain a credential or personal data; publish safe metadata only");
  }
}
...
export function createAgenticShipMcpServer(projectRoot, options = {}) {
  ...
  async function handleRequest(request) {
    ...
    if (request.method === "initialize") {
      if (notification || !request.params || typeof request.params.protocolVersion !== "string") {
        return protocolError(request.id, -32602, "initialize requires protocolVersion");
      }
      initialized = true;
      return {
        jsonrpc: "2.0",
