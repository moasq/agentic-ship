...
Reference for the workspace-health skill. Read this before writing any command into a skill,
a README, or a chat reply.

## The rule

**Node is the only runtime Agentic Ship assumes.** Every Agentic Ship operation is a Node script
...
Keep it local, or gate it behind a local fork. `pnpm health` prints this reminder on
`win32` automatically.
