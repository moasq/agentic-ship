...
description: Use when connecting, authorizing, provisioning, resuming, verifying, canceling, or diagnosing an external service such as Convex, Stripe, GitHub, Resend, PostHog, Netlify, Vercel, or Cloudflare, especially when browser consent or another human-only step must pause an agent running in Claude Code, Codex, Cursor, Hermes, or OpenClaw.
---
...
3. On yes, execute the chosen decision steps and then every step in
   ...
   whose vendors ship CLI OAuth (Stripe pairing codes, Netlify, Vercel, or Cloudflare browser login,
